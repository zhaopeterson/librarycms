...
if(mysqli_affected_rows($conn)==1){
	...
	';
		...
		echo '<p align="center">There is not enough books in the system for your to reserve.</p>';
		exit();
	}
	$affected2=0;
		//prepare to insert to book history table
		$stmt2=$conn->stmt_init();
		$sql_bh="INSERT INTO book_history (book_id, member_id, member_quantity, reserve_date, due_date,inventory_current) VALUES (?,?,?,?,?,?)";	
		$stmt2->prepare($sql_bh);
		$stmt2->bind_param('iiissi', $book_id, $member, $qty, $reserve_date, $due_date,$new_bookinventory);
